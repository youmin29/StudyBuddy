...
### 2026-05-26 — v1.1.5 클라우드 동기화 병합 방식으로 수정

**🚑 버그 수정**
- 여러 기기에서 클라우드 업로드 시 기존 데이터가 덮어씌워지던 문제 수정
- 투두: id 기준 upsert로 새 항목만 추가, 다른 기기 데이터 보존
- 플레이리스트: 클라우드에 이미 같은 URL이 있으면 스킵, 없는 것만 추가
- 로그아웃 후 재업로드 시 중복 생성 방지

---

### 2026-05-26 — v1.1.4 플레이어 기능 개선 및 버그 수정

**🚑 버그 수정**
- 앱 재시작 시 로그인 세션이 풀리던 문제 수정 — Supabase 세션을 `localStorage` 대신 `userData/auth.json` 파일에 저장하도록 변경 (랜덤 포트로 인한 origin 변경 문제 해결)

**✨ 플레이어 기능 정상화**
- **셔플**: YouTube 플레이리스트 로드 시 셔플 ON/OFF 토글 작동. 단일 영상일 땐 버튼 비활성화
- **다음 곡**: 플레이리스트면 `nextVideo()`, 단일 영상이면 라이브러리 다음 저장 항목으로 이동
- **반복 재생**: 플레이리스트 전체 반복 또는 단일 영상 무한 반복 토글
- 셔플·반복 활성화 시 버튼 강조 표시로 현재 상태 시각적 확인 가능
